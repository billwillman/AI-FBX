...
def _CreateSkin(fbxManager, scene, mesh: FbxMesh, meshNode, vertexBoneDatas, useBoneIndexData, vertexDatas, RootNode):
    ...
    try:
        ...
        for i in range(0, N1, 1):
            ...
            N2 = len(boneWeightDatas)
            for j in range(0, N2, 1):
                if abs(boneWeightDatas[j]) >= _cMinWeight and boneWeightDatas[j] > 0:
...
def AddSkinnedDataToMesh(fbxManager, scene, mesh, meshNode, vertexBoneDatas, bonePosDatas, boneRotDatas,
    ...
    ## 顶点蒙皮
    _CreateSkin(fbxManager, scene, mesh, meshNode, vertexBoneDatas, useBoneIndexData, vertexDatas, RootNode)
    ## 更换骨骼节点名(执行放最后)
    if RootNode != None:
